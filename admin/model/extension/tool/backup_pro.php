...
//								fwrite($fp, "SET FOREIGN_KEY_CHECKS=0;\n\n");
	...
	public function getNoOfParts() {
		$query = $this->db->query("SELECT MAX(`part`) as `parts` FROM " . DB_PREFIX . "backup");
		if($query->num_rows) {
			return $query->row['parts'];
		} else {
			return 0;
		}
	}
	...
	public function housekeep404s() {
		...
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "404s_report` WHERE `link` LIKE \"%'%\"");
		
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "404s_report` WHERE `date` <= '" . date('Y-m-d H:i:s', strtotime('- 3 MONTHS')) . "'");
