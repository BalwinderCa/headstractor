...
!function ($) {
    ...
    BootstrapTable.prototype.initTable = function () {
        ...
        if (!this.$header.find('tr').length) {
            this.$header.append('<tr class="filters"></tr>');
        }
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            html.push('<th',
                ...
                '>');
            if (that.options.filter && column.filtrable) {
                if (!column.filtrableList)
                    html.push('<input name="filter-input" id="flt-field'+column.field+'" field="'+column.field+'" type="text" value="'+column.filter+'" class="form-control" placeholder="'+column.title+'">');
                else {
                    $.ajax({
                        async: false,
                        type: 'POST',
                        url: that.options.filter_url,
                        data:  { 'col': column.field},
                        cache: true,
                        success: function (res) {
                            var vals = '<option value="" selected>--- '+column.title+' ---</option>';
                            $.each(res, function (i, value) {
                                vals += '<option value="'+value['value']+'">'+value['text']+'</option>';
                            });
                            html.push('<select name="filter-input" id="flt-field'+column.field+'" field="'+column.field+'"class="form-control">'+vals+'</select>');
                        }
                    });
                }
            }
            else 
{
            ...
            if (that.options.sortName === column.field && that.options.sortable && column.sortable && !that.options.filter) {
        ...
        this.$container.off('click', 'th').on('click', 'th', function (event) {
            if (!that.options.filter && that.options.sortable && $(this).data().sortable) {
                that.onSort(event);
            }
        });
        ...
        this.$container.find('input').off('keyup').on('keyup', function (event) { 
                clearTimeout(timeoutId); // doesn't matter if it's 0
                timeoutId = setTimeout(function () {
                    that.onFilterKeyUp(event); 
                }, that.options.searchTimeOut);
        });

            this.$container.find('select').off('change').on('change', function (event) {
            clearTimeout(timeoutId);
            timeoutId = setTimeout(function () {
                that.onFilterSelectList(event);
            }, that.options.searchTimeOut);
        });
    ...
    BootstrapTable.prototype.onFilterSelectList = function (event) {
        var $this = $(event.currentTarget);
        var $th = $this.parent();
        var indx = getFieldIndex(this.options.columns, $th.data('field'));

        if (this.options.columns[indx].filter != $this.val()) {
            this.options.columns[indx].filter = $this.val();
            this.options.pageNumber = 1;
            this.updatePagination();
        }
    };

    BootstrapTable.prototype.onFilterKeyUp = function (event) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showFilter) {
            html.push(sprintf('<button class="btn btn-default" type="button" name="filter" title="%s"><i class="fa fa-filter"></i>', this.options.formatFilter()));
        }
        ...
        this.$toolbar.find('button[name="filter"]')
            .off('click').on('click', $.proxy(this.filter, this));
...
if (this.options.filter) {
	...
	$.each(this.options.columns, function (i, column) {
	if (column.filter) {
		filter['filter_'+column.field] = column.filter;
	}
	});
        $.extend(data, filter || {});
}
    ...
    BootstrapTable.prototype.resetHeader = function () {
        ...
        setTimeout(function () {
            ...
            that.$header.find('th').each(function (i) {
                ...
                var edit = that.$header_.find('th').eq(i).find('input[name="filter-input"]');
                var field_index = getFieldIndex(that.options.columns, that.$header_.find('th').eq(i).data('field'));
                if (edit !== undefined) edit.val(that.options.columns[field_index].filter);
            });
	...
	if (!checked) this.options.columns[index].filter = '';
        this.forceResetHeader = true;
        this.initHeader();
        this.initSearch();
        this.initPagination();
        this.initBody();
    ...
    BootstrapTable.prototype.resetView = function (params) {
        ...
        this.$container.find('[name="btSelectAll"]').prop('checked', this.$selectItem.length > 0 &&
        ...
        if (this.options.showHeader && this.options.height && (this.forceResetHeader || !this.options.filter)) {
            this.forceResetHeader = false;
            this.resetHeader();
        } 
    ...
    BootstrapTable.prototype.filter = function (params) {
