...
class ModelExtensionPaymentDivido extends Model {
	...
	public function getProductSettings($product_id) {
        $product_id = (int)$product_id; // Already sanitized, just making explicit
        return $this->db->query("SELECT `display`, `plans` FROM `" . DB_PREFIX . "divido_product` WHERE `product_id` = '" . $product_id . "'")->row;
    }
	...
	public function getLookupByOrderId($order_id) {
       $order_id = (int)$order_id; // Cast to integer to prevent SQL injection
       $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "divido_lookup` WHERE `order_id` = " . $order_id);
       return $query->row;
    }
